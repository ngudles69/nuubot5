...
// Reconcile creates one coherent Account snapshot through the selected temporary path.
func (a *Account) Reconcile(nowMS uint64, forced bool) (Snapshot, bool, error) {
	if a.config.Recon == "recon2" {
		return a.reconcile2(nowMS, forced)
	}
	return a.reconcile(nowMS, forced)
}
