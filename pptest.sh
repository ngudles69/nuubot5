...
bot_id="${2:-9}"
if [[ $# -gt 2 ]]; then
    echo "usage: bash pptest.sh [sweep_id] [bot_id]" >&2
    exit 2
fi
for value in "$sweep_id" "$bot_id"; do
...
result_log="$log_dir/nuubot5-pptest-s${sweep_id}-b${bot_id}-${stamp}.log"
suite_json="${result_log%.log}.json"
profile_dir="$repo_root/workspace/perf/profiles/pptest-s${sweep_id}-b${bot_id}-${stamp}"
mkdir -p "$profile_dir" || exit 1
profile_prefix="$profile_dir/run-001"
...
for ((run = 1; run <= runs; run++)); do
    ...
    run_json="$(cd "$repo_root" && timeout 120s "$btrunner" "$sweep_id" "$bot_id" -pp "$profile_prefix")"
    status=$?
    ...
    if [[ $status -eq 0 && -n "$run_json" ]]; then
