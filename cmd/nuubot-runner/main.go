...
func main() {
	...
	if err == nil {
		err = live.Execute(context.Background(), options)
	}
	if err != nil {
...
func parseArguments(args []string) (live.Options, error) {
	var options live.Options
	if len(args) != 2 {
		return options, fmt.Errorf("usage: %s <sweep_id> <bot_id>", program)
	}
